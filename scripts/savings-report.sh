...
#
# Usage:
#   scripts/savings-report.sh [--corpus-dir DIR] [--corpus-profile small|heterogeneous]
#       [--work-dir DIR] [--profile raw|zstd|both]
#       [--catalog-path PATH] [--index-path PATH] [--model-path PATH]
#       [--temporary-peak-bytes BYTES]
#
#   --corpus-dir DIR  an existing, non-empty corpus to read (never modified)
#                     (default: a deterministic probe in a temp dir)
#   --corpus-profile P  generated corpus profile: small (default) or
#                       heterogeneous (about 4 MiB, generated by the internal
#                       qualificationcorpus package)
...
#   --catalog-path P  existing catalog file/directory to measure read-only
#   --index-path P    existing search index file/directory to measure read-only
#   --model-path P    existing model file/directory to measure read-only
#   --temporary-peak-bytes N  caller-supplied peak temporary bytes
...
usage() {
    echo "usage: $0 [--corpus-dir DIR] [--corpus-profile small|heterogeneous] [--work-dir DIR] [--profile raw|zstd|both] [--catalog-path PATH] [--index-path PATH] [--model-path PATH] [--temporary-peak-bytes BYTES] [--keep]" >&2
    exit 2
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --temporary-peak-bytes) TEMPORARY_PEAK_BYTES="$2"; shift 2 ;;
        --keep)       KEEP=1;          shift ;;
        -h|--help)    usage ;;
...
case "$CORPUS_PROFILE" in
    small|heterogeneous) ;;
    *) echo "error: --corpus-profile must be small or heterogeneous" >&2; usage ;;
esac
if [[ -n "$CORPUS_DIR" && "$CORPUS_PROFILE" != small ]]; then
    echo "error: --corpus-profile applies only when --corpus-dir is omitted" >&2
    exit 2
fi
...
DEPLOYMENT_ARGS=()
if [[ -n "$CATALOG_PATH" ]]; then DEPLOYMENT_ARGS+=( -catalog-path "$CATALOG_PATH" ); fi
if [[ -n "$INDEX_PATH" ]]; then DEPLOYMENT_ARGS+=( -index-path "$INDEX_PATH" ); fi
if [[ -n "$MODEL_PATH" ]]; then DEPLOYMENT_ARGS+=( -model-path "$MODEL_PATH" ); fi
if [[ -n "$TEMPORARY_PEAK_BYTES" ]]; then DEPLOYMENT_ARGS+=( -temporary-peak-bytes "$TEMPORARY_PEAK_BYTES" ); fi
...
if [[ "$GENERATED_CORPUS" -eq 1 ]]; then
    if [[ "$CORPUS_PROFILE" == heterogeneous ]]; then
        GENERATED_CORPUS_MANIFEST="$TMP_BASE/generated-corpus.qualification.json"
        go run -tags=savingsreport "$RUNNER" \
            -generate-corpus "$CORPUS_DIR" -generate-size-mib 4 \
            -generate-seed restoreweave-phase5-heterogeneous-v1 \
            -manifest-out "$GENERATED_CORPUS_MANIFEST"
    else
        mkdir -p "$CORPUS_DIR"
    fi
fi
...
if [[ "$CORPUS_PROFILE" == heterogeneous ]]; then
    echo "profile: heterogeneous (internal qualification corpus generator)"
else
...
run_profile() { # profile dirname
	...
	local runner_args=(-profile "$profile" -repo "$WORK_DIR/$dir" -corpus "$CORPUS_DIR" \
		-manifest-out "$manifest_path" -evidence-out "$WORK_DIR/$profile.candidate-evidence.json")
	if [[ ${#DEPLOYMENT_ARGS[@]} -gt 0 ]]; then
		runner_args+=("${DEPLOYMENT_ARGS[@]}")
	fi
	go run -tags=savingsreport "$RUNNER" "${runner_args[@]}"
...
mkdir -p "$WORK_DIR"
if [[ -n "$GENERATED_CORPUS_MANIFEST" ]]; then
    cp "$GENERATED_CORPUS_MANIFEST" "$WORK_DIR/generated-corpus.qualification.json"
fi
