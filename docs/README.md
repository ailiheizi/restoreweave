...
| Exact repository, protection records, plan/apply executor, publication, and clean restore | The admitted development profile has implemented and tested Ed25519 prepared/commit records, signed processor-attempt and portable-fact children, portable subject mapping and body attachments, catalog-free restore, independent-anchor clean-install import/reader, recovery tokens, cross-process fencing, unknown-outcome reconciliation, raw/zstd corruption and relocation handling, explicit host-owned repair, and identity-preserving copy-forward profile migration with signed clean-reader restore evidence. The raw and zstd repositories remain development/candidate profiles; production qualification, encryption release admission, GC qualification, broader per-field provenance, sparse extent maps, formal async retry/reprocessing qualification, and external reacquisition remain open. A separate encrypted zstd profile supplies experimental key-boundary evidence only |
