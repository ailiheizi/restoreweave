...
| Storage scheme and capacity | The current profile has one configured repository; a future named scheme may bundle location, driver, exact deduplication, and compression for new placements. Capacity is an optional read-only report and may be `UNKNOWN` with a reason; it never blocks exact saving |
...
| `paths.models` | Installed local model/runtime bundle root | Platform XDG data path |
...
| `api.enabled` | Enable the bounded local HTTP adapter | `false` |
| `api.listen` | HTTP listen address; loopback unless an outer security layer is qualified | `127.0.0.1:4534` |
...
models = "/data/restoreweave/models"
...
| `local-zstd-v1` | candidate | Embedded compression, whole-file dedup, corruption rejection, relocation, profile isolation, explicit repair, copy-forward migration with independently reopened source/target clean readers and rollback preservation, including a real pre-publication process-crash/retry boundary, mechanism-separated savings, and `NON_DESTRUCTIVE_ONLY` verified root/inventory candidate planning exist and are tested; encryption admission, broader crash/migration rollback and reader-closure qualification, representative corpus/host evidence, packaging, and a dated release-engine decision remain. Chunking is optional and unselected; destructive GC is disabled and is not an MVP gate. The separate `local-zstd-encrypted-v1` experimental profile has host-owned AES-256-GCM/key-provider, key-rotation copy-forward, and real pre-publication process-crash/retry evidence but is not available through the persisted config/daemon or a release default |
...
| Read-only capacity and per-item index status | implemented and tested for the current profile | `status.get` reports measured or explicit unknown capacity; `content.list`, `namespace.list`, and search results expose calculated tag/lexical/semantic coverage without creating a durable status table. Missing optional projections never make exact content unreadable |
| Local ONNX/BGE encoder | implemented and tested for the opt-in worker/component profile | The supervised worker admits the pinned bundle and produces real vectors. Focused tests cover the fixed development installer, the real supervised ONNX/BGE + zvec daemon/CLI semantic E2E passes in the provisioned Darwin arm64 environment (with corresponding opt-in Linux arm64 evidence), and a dated Darwin run records bounded Chinese/English recall, latency, bundle-size, and RSS evidence. A candidate-only offline artifact assembler deterministically combines supplied daemon/CLI/Web builds and a retained semantic archive with verified checksums and separated license/SBOM evidence; it is not a supported installer. The browser adapter build/E2E covers honest semantic-unavailable degradation; no browser install/restart/rebuild/real-query evidence is claimed. Supported clean-host packaging and broader release qualification remain open |
