...
> **Profile status:** This is an extended qualification catalog. The first release gate is the single-node Linux/NAS managed-archive profile in [MVP and Operator Contract](mvp-and-operator-contract.md): one local or mounted root, an honest generic capture profile, one mature exact deduplicating and compressing repository engine, deterministic identification, bounded default metadata/text processing, durable whole-subject tags and notes, hybrid lexical/structured search, the bundled local ONNX/zvec semantic profile, saved views, export manifests, a portable namespace and signed RRF publication commit, CLI, and read-only MCP. Platform- and engine-specific implementations qualify separately and are never global gates. REST, WebUI, resident scheduling, remote AI, CLIP, multimodal search, multiple placements, A2A, P2P, public plugin protocols, alternate fidelity, multitenancy, cold media, and enterprise custody are later profiles.
...
- Export-manifest and `FileAccess` implementation, principal and export-root binding, raw-name and sparse-file semantics, cache and revocation profile, and the external-tool boundary. RestoreWeave has no bundled mount service.
- Default processor profiles, lexical/structured and local-semantic index backends and generations, extraction and semantic coverage, subject/segment resolution, rebuild behavior, authorization mapping, the MVP tag/note/description schema, and authenticated portable annotation/description records. Alternate embedding spaces, distributed vector services, multimodal indexes, and their purge behavior apply only to later profiles.
...
- Export and `FileAccess` qualification covering raw names, hard links, sparse files, symlinks, exact byte identity, authorization expiry, and clean-install materialization. Mount, kernel-cache, and unmount behavior belong to the external tool and are not RestoreWeave release gates.
...
- Export materialization time, `FileAccess` first-byte and range-read latency, repository request and byte amplification, process memory, concurrent content-handle limits, and authorization residual-access bounds.
...
- Default hybrid search over paths, types, checksums, duplicate groups, declared metadata, common media metadata, durable tags and notes, processing state, extracted text, and the pinned local text-embedding generation, with stable subject resolution and rebuild qualification.
...
- Export-manifest materialization over the authenticated original-path namespace, binding one principal, one explicit destination, and one immutable manifest; verifying selected representations, names, metadata degradation, exact bytes, and per-item/final receipts. Optional filesystem projections qualify separately and are not an MVP gate.
...
P2P, source-only omission, non-exact authority, public plugin execution, WebUI, CLIP, additional vector spaces or multimodal search, remote AI or an agent harness, external enrichment as a recovery dependency, MCP mutation grants, REST, A2A, learned codecs, application-consistency claims, multitenancy, recurring-daemon or scheduler claims, multiple placements, cold media, destructive automatic garbage collection, and `RESTOREWEAVE_PACKS` are `DEFERRED` or `PROHIBITED` in this profile even though later-phase requirements exist. The pinned local text embedding and zvec generation are part of the default discovery profile.
...
20. Export-manifest materialization passes subject/representation binding, authorization, destination collision, metadata-degradation, streaming and random-read, exact-byte verification, restart/retry, and per-item/final receipt qualification for the declared profile.
