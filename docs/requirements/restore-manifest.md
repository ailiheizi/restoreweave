...
Version 2 adds an append-only, complete-state successor lineage without changing the exact publication. The first v2 child has sequence 1 and no predecessor, or sequence 2 may succeed one existing v1 child treated as the immutable sequence-1 root. Every later child binds the immediately preceding signed closure digest and contains every previously authenticated terminal attempt byte-for-byte plus at least one newly appended terminal attempt. A skipped sequence, two records at one sequence, two successors of one predecessor, a missing predecessor, non-monotonic signing time, or a successor that removes or rewrites an earlier attempt invalidates processor-provenance reading while exact discovery and restore remain usable. Publishing the same complete state is idempotent. This lineage permits durable retry results to be reconciled; it does not itself schedule retries or authorize an async worker to retry without host-owned intent, idempotency, fencing, unknown-outcome reconciliation, and retry ceilings.
