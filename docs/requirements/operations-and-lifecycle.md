...
`RW-MVP-1` owns the subject bindings and generation truth for a rebuildable local path, metadata, type, checksum, duplicate, common-media-metadata, durable tag/note, and extracted-text index. It also owns durable whole-subject tag and plain-text note CRUD plus portable export/import independently of index storage. It does not make the index recovery authority. Embeddings, CLIP, vector or multimodal ranking, external enrichment, collections, ratings, relationship graphs, typed segment annotations, and machine-suggestion review remain later or external capabilities.
