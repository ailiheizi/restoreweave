...
Keep two proofs separate. The **recovery proof** does not need a WebUI. The **product proof** is one search, one item page, and one restore; it must not wait for a live FUSE mount or three media servers.
...
RestoreWeave does not own a kernel FUSE adapter as a product job. `plan.restore` writes a real directory; `content.open`/`read` and `FileAccess` hand out exact bytes. rclone, sshfs, SMB on a NAS, WebDAV helpers, and similar tools already turn a directory or a read stream into a mount. That work stays outside `restoreweaved`. The in-tree go-fuse adapter is leftover Linux code, not a gate and not an experience plane.
...
- Direct `go.mod` dependencies: `modernc.org/sqlite`, `hanwen/go-fuse/v2` v2.11.0, `google.golang.org/grpc`, `modelcontextprotocol/go-sdk`, `spf13/cobra`, `golang.org/x/sys`.
...
- Fake directory CAS remains the in-tree `RepositoryDriver`. Darwin/Unix CLI probes for Restic and Kopia can pass when those binaries are present; that is not an engine selection.
- Linux bubblewrap execution remains open where isolated heavy parsers are wanted. Live `/dev/fuse` is not a RestoreWeave product gate. The in-tree FUSE adapter may stay unused.
...
| **Pinned library, types private** | Narrow adapter whose types must not enter portable records | go-fuse, SQLite FTS5, Cobra, MCP SDK, grpc-go |
...
| **Foreign mount tools** | Operator wants a folder in Finder/Explorer/`ls` | Restore a snapshot (or read via `FileAccess`) and let rclone/sshfs/SMB/WebDAV mount it. Do not grow a RestoreWeave FUSE daemon. |
...
| Repository | Fake CAS now; Kopia/Restic qualification | Replace the fake CAS only after remaining Driver gates, including NAS/S3 | Serialize Kopia or Restic types into portable records |
...
| Search | SQLite FTS5 generations | Keep the schema private; later Tantivy/Qdrant only after measurement | Publish FTS tables as ABI |
| Namespace / file egress | `SnapshotTree` / `FileAccess` / `plan.restore` | Keep exact bytes and namespace facts. Let foreign tools mount a restored tree or a read handle. | Own `/dev/fuse`; leak go-fuse types; enable `allow_other`; treat “point Navidrome at a mount” as catalog proof |
...
| Inbox / generic UI | None in tree | Thin read-only shell over the command ABI once that ABI exists. Do not wait for live FUSE. | Adopt File Browser as a writable manager of the source tree; postpone the Inbox shell until three media servers work |
| Mixed indexer analogue | sist2 | External Processor or design reference | Let a second index become recovery authority |
| Music | Navidrome and Subsonic clients | Prefer an OpenSubsonic facade or in-page `FileAccess` preview. A read-only mount is an optional Linux smoke test after live FUSE qualifies, same UID, data/cache off the mount. | Embed a media server or library scanner in `restoreweaved`; treat Navidrome’s database as portable progress |
...
| Video | Jellyfin server and jellyfin-web | Later than music/books. Keep NFO-next-to-file and adjacent artwork off; transcode only in a qualified sandbox, never on the FUSE root. | Let Jellyfin own storage, exact bytes, or the daily catalog |
...
1. Finish remaining repository gates (independent SHA-256 readback, then NAS/S3). Replace the fake CAS only after those gates pass. Isolated processors may use Linux bubblewrap later. Do not spend further work making RestoreWeave a FUSE server.
...
6. If an operator wants a folder, restore the snapshot (or expose `FileAccess` bytes) and let an existing mount tool attach it. Do not qualify RestoreWeave by pointing Navidrome at a FUSE mount. Jellyfin stays later than music/books.

Do not start a full custom music, reader, or photo application in this repository to “have a frontend.” Do not enable `allow_other` so a containerized client can see the mount; run the client as the mounting principal, or use a protocol facade.

## 6. What would reverse a row

- Read-only FUSE is **expected** not to carry play/read progress back to `SubjectRef`. That is the default, not a surprise. Add a protocol facade or write progress from the Inbox item page. Do not implement a media server to close the gap.
- If a live Linux mount plus a same-UID Navidrome scan meets the playable-latency metrics in [Ecosystem and Vertical Applications](../requirements/ecosystem-and-vertical-apps.md) §10 **and** does not build a second catalog, the optional smoke test may be promoted. `allow_other` still does not turn on.
...
**Held.** Do not embed a player, reader, or media server in `restoreweaved`. A client-private database is not recovery authority. File Browser must not own writable source-tree semantics. Stars do not select Kopia and do not authorize GPL linking. The core must restore without a WebUI. The fake CAS still waits on Driver gates. Tika/ffprobe stay isolated.

**Reversed or tightened.**

- “Point existing clients at a read-only FUSE snapshot” is not the default introduction path. On Linux, same UID, with data/cache off the mount and library-rewrite options off, Navidrome and Komga can scan and direct-play; Jellyfin is narrower. None of them write progress back to `SubjectRef`. This Darwin host has no `/dev/fuse`, so the path is not exercisable here. FUSE remains file-shaped egress.
