...
| 2 | **Implemented and tested for the admitted bounded model:** non-mutating ingest and restore planning, digest-bound apply, source/config/manifest/destination revalidation, same-plan replay, committed-publication reconciliation, verified completed-restore reconciliation, typed ingest-protection revision with reinspection/recomputation, fail-closed apply for blocked plans, per-subject degraded processor reporting, durable append-only processor terminal outcomes, a signed complete-state successor lineage that retains later outcomes without rewriting earlier attempts, and bounded automatic retry of the same signed processor plan with idempotency, leases/fencing, reconciliation, restart/resume, retry ceilings, and pre-execution retry-input/publication/target binding checks. Invalid retry input is claimed through the normal lease state transition and then fails closed without invoking the processor. The qualification matrix exercises two-worker competition, expired-lease fencing, a real process crash followed by cross-process lease takeover, unknown-outcome reconciliation, and exact restore. The exact apply Job remains successful after post-publication processor/index failure. **The Phase 2 gate is closed for this bounded contract.** | No Phase 2 blocker remains. Arbitrary manual, rerouted, or general reprocessing stays disabled and requires a separately reviewed signed successor contract; production repository and release-host qualification belong to Phases 5 and 7 |
...
| 4 | Durable description revisions and source-aligned segments have portable authenticated recovery with config/provider/segmentation profile bindings; the complete stated lexical/structured field scope, typed filters, segment-hit provenance, measured coverage, host-owned fusion, generation binding, honest semantic degradation, focused tests for the fixed development installer, and a real supervised Darwin arm64 ONNX/BGE + zvec daemon/CLI E2E (with corresponding opt-in Linux arm64 evidence) are implemented and executed. A dated Darwin arm64 component run records the exact bundle/profile, eight-case Chinese/English recall, startup and query latency, bundle bytes, and observed worker RSS. A deterministic candidate-only offline artifact assembler now packages supplied daemon/CLI/Web builds and semantic archive with checksums and separated license/SBOM evidence. The browser adapter build/E2E covers semantic-unavailable degradation and does not claim browser install/restart/rebuild/real-query execution | Exercise that candidate artifact on clean supported hosts, complete the release SBOM/redistribution review, run broader representative recall/concurrency/peak-memory evidence on supported Linux/NAS hosts, and make the qualified default broker lexical + structured + semantic |
| 5 | `local-zstd-v1` remains an opt-in, no-Docker-Compose unencrypted candidate with whole-file SHA-256 deduplication, checksummed zstd payloads, transparent decode-and-hash verification, physical-byte placement receipts, profile mismatch protection, relocation/corruption tests, explicit host-owned repair and copy-forward migration that preserves repository identity, independently reopened source/target signed clean-reader verification with target-tamper rejection and source rollback restore, concurrent no-replace placement, catalog-free signed restore, fail-closed logical/duplicate/compression/physical/overhead/net accounting, and `NON_DESTRUCTIVE_ONLY` verified placement inventory/root/active-lease candidate planning. A separate `local-zstd-encrypted-v1` experimental profile proves host-owned AES-256-GCM key resolution, missing/wrong-key fail-closed behavior, encrypted readback, relocation, and key-rotation copy-forward—including a real pre-publication process crash followed by retry and independently reopened source/target readers—without changing portable records or the config default | Encryption release admission and daemon/secret-provider wiring, broader crash qualification, representative corpus measurements, full migration rollback/reader-closure qualification, supported packaging/host evidence, and a dated release repository decision. Whole-file deduplication is sufficient; optional chunking is unselected, and destructive collection is disabled rather than an MVP gate. Phase 5 remains open |
...
The current `local-zstd-v1` candidate implements items 1-3 with whole-file zstd and no external service, plus explicit host-owned repair and copy-forward profile migration. The copy-forward path retains the source repository identity, copies the known snapshot manifest placement, rejects unknown or leftover source entries, and has bounded source/target signed clean-reader list/verify/restore evidence; both readers can be reopened after the catalog is closed, target corruption is rejected, and the source remains independently restorable as rollback authority. Failed migrations stage beside the target without publishing a partial repository, and injected payload/record/pre-publish interruption boundaries plus a real pre-publication process-crash/retry boundary can be retried safely. A raw fallback remains readable when the experimental zstd source profile is unavailable. Core-owned raw/zstd drivers also expose optional capability/health, read-only target validation, and placement-estimation seams. The reports keep the embedded reader, whole-file chunking, `NONE` encryption, `NOT_REQUIRED` key state, unknown capacity, and non-receipt physical estimates explicit; they do not initialize, mutate, or authorize placement. A separate `local-zstd-encrypted-v1` candidate exercises an injected host `KeyProvider`, non-secret key references, AES-256-GCM protection around zstd payloads, authenticated decode-and-hash readback, missing/wrong-key health states, clean-reader dependency, relocation, corruption, and key-rotation copy-forward. Its migration test now kills a real process after encrypted target payload verification, proves that no live target was published, then retries and reopens source and target with their respective key providers while old/wrong keys fail closed. Portable recovery records remain independently readable and secrets never enter plans or portable records. Neither profile is accepted by the generated configuration as a release default. This is enough to test exactness, repair, bounded migration reader closure, codec-loss fallback, encryption/key-boundary behavior, and gather corpus evidence, not enough to close the phase: broader crash/migration rollback qualification, representative operator corpus measurements, supported packaging/host qualification, encryption release admission and daemon/secret-provider wiring, final repository decision, and complete release reader closure remain open. Whole-file deduplication is sufficient for the MVP; optional chunking stays unselected, and destructive GC remains disabled with only `NON_DESTRUCTIVE_ONLY` planning. The generated config therefore remains on `directory-cas-dev-v1` until a release profile is qualified.

Phase 5 does not begin with a selected repository brand. Every candidate is scored against the same dated qualification record: complete `RepositoryDriver` operations, immutable logical identity, exact readback, crash/corruption behavior, encryption and credential handling, non-destructive reachability and retention ownership, repair, relocation, migration/rollback, supported-platform packaging, independently installable reader closure, measured net savings, and license/support risk. Exactly one tuple may become the release default only after all mandatory gates pass; if none passes, `RW-MVP-1` remains blocked rather than silently promoting `local-zstd-v1`, Kopia, Restic, or another candidate.
...
2. Lossless compression reads back to the original digest and length; if a selected repository profile uses chunking, its chunks independently pass the same readback and corruption requirements.
